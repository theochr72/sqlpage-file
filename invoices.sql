...
SELECT 'form' AS component,
       ...
       'Rechercher' AS validate,
       'search' AS validate_icon,
       'azure' AS validate_color;

SELECT 'select' AS type, 'supplier' AS name, 'Fournisseur' AS label,
...
SELECT 'date' AS type, 'date_from' AS name, 'Du' AS label,
       $date_from AS value, 3 AS width;

SELECT 'date' AS type, 'date_to' AS name, 'Au' AS label,
...
-- ── Actions groupees (pending_review uniquement) ─────────────────────────────

SELECT 'button' AS component, 'end' AS justify, 'sm' AS size
 WHERE $status = 'pending_review';

SELECT 'Tout valider (≥80%)' AS title,
       'green' AS color,
       'checks' AS icon,
...
SELECT 'Tout rejeter (< 20%)' AS title,
       'red' AS outline,
       'circle-x' AS icon,
...
SELECT 'table' AS component,
       TRUE AS sort,
       TRUE AS search,
       'Confiance,Montant' AS align_right,
       TRUE AS hover,
       TRUE AS striped_rows,
       TRUE AS small,
       'Aucune facture ne correspond a vos filtres.' AS empty_description;
